...
docker restart sgd-nginx  # si da 502
scripts\seed_full_restore.bat  # UN SOLO COMANDO: sync AD + roles + documentos
```
El script `seed_full_restore.bat` restaura 757 usuarios + 724 usuario_roles + 10 documentos.
...
**Sesión 37 (2026-06-19)**: **R3 FASE 1 CERRADA**: 7 tablas nuevas + enum `TipoTarea` extendido (LIBERACION, CORRECCION) + columna `usa_dias_habiles` + data migration de semaforizacion (REVISION/APROBACION 4/7/10, plazo 10). 9 archivos de modelos nuevos (`backend/app/models/{tarea,bitacora_timeline,notificacion,documento_reemplazo,documento_alcance_difusion,tarea_observacion,proceso}.py`), `models/__init__.py` actualizado, `semaforizacion_tarea.py` extendido. 7 archivos de tests nuevos (51 tests). Conftest.py actualizado (seed 6 tipos semaforo + PRAGMA foreign_keys=ON). Migración Alembic `r3_fase1_s37` aplicada manualmente a DES (alter type + 7 create table + update values + insert 2 new semaforo rows). HEAD Alembic actualizado. 300/300 tests PASS.
