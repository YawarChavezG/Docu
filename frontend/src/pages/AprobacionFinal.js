/**
 * pages/AprobacionFinal.js — Pantalla de Aprobación Final (US-3.04)
 * Para aprobadores finales
 */
import { historialAprobacion } from '../data/bitacora.js'

export const page = {
  init() {
    window.Alpine?.data('aprobacionFinalPage', () => ({
      obs: '',
      mostrarObs: false,
      ...
      iniciarAccion(tipo) {
        ...
        window.authModal?.abrir({
          titulo: esAprobar ? '✓ Aprobar Documento (Final)' : '↩ Devolver con Observaciones',
          icono: esAprobar ? '✓' : '↩',
          mensaje: esAprobar
            ? 'Va a firmar digitalmente la <strong>aprobación final</strong> de <strong>MAN-LOG-003 v01</strong>. Esta acción queda registrada en la bitácora del sistema y el documento pasará a publicación.'
          ...
          onSuccess: () => {
            if (esAprobar) {
              window.toast('✅ Firma digital registrada. Documento aprobado y enviado a publicación.', 'success')
            } else {
      ...
      delegar() {
        window.delegarTareaModal?.abrir({
          ...
          de: 'Luis Mamani (Actual)',
          a: 'Pedro Quispe (Nuevo)',
          onConfirm: () => {
  ...
  template: /* html */`
<div x-data="aprobacionFinalPage" class="animate-fade-in-page">

  <div class="flex items-center justify-between mb-3.5 flex-wrap gap-2">
    <div class="flex items-center gap-2.5 flex-wrap">
      <h1 class="text-sm font-bold text-slate-800">Tarea - Aprobadores — MAN-LOG-003 Manual de Logística y Distribución</h1>
      <span class="badge badge-blue">MAN-LOG-003 · v01</span>
      <span class="badge badge-purple">En aprobación final</span>
    </div>
  ...
  <div class="card-base mb-4">
    <div class="section-header">Archivos a aprobar (Office 365 Web)</div>
    ...
    <div class="mt-3 bg-blue-50 border border-blue-200 rounded-lg p-2.5 text-[11px] text-blue-800">
      📌 <strong>Nota:</strong> Todos los revisores técnicos han concluido. Usted es el aprobador final.
    </div>
  </div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
    ...
    <div class="lg:col-span-2 space-y-4">
      ...
      <div class="card-base">
        <div class="section-header mb-3 pb-2">Mi acción como Aprobador Final</div>
      ...
      <div class="card-base">
        ...
        <div class="space-y-2.5 text-[11.5px] text-slate-600">
          ...
          <div class="flex justify-between">
            ...
            <span class="badge badge-amber">5 días restantes</span>
          </div>
        </div>
      </div>
    </div>
